...
public class JsonUtils {
	...
	public static JsonUtils getInstance(){return Instance.mJsonUtils;}
	...
	/** 获取Field[] */
	public Field[] getiField(Class<?> c) {
		Field[] f = null;
		try {
			f = c.getFields();
	 ...
	 */
	public List<?> toList(JSONArray array, Class<?> c) {
		...
		for (int i = 0; i < array.length(); i++) {
			...
			try {
				ob = c.newInstance();
				ob = toClass(object, ob, fs, c.getName());
				if (null != ob)
					datas.add(ob);
			} catch (Exception e) {
	 ...
	 * @return List<Class对象> 错误 return null
	 */
	public List<?> toList(String arr, Class<?> c) {
		List<Object> datas = new ArrayList<Object>();
		if (TextUtils.isEmpty(arr)|| c == null || isBaseType(c)
				||c.isArray() || c.isEnum() || c.isInterface())
			return datas;
		JSONArray array = parseArray(arr);
		if(null == array)
			return datas;
		Field[] fs = getiField(c);
		if (null == fs)
	...
	private Object toClass(JSONObject object, Object ob, Field[] fs, String className) {
			...
			try {
				...
				if (type.equals(String.class)) {
					...
					if (str.getClass().equals(JSONArray.class)) {
						Object child = toList((JSONArray) str, type);
						if (child != null)
							f.set(ob, child);
					}
					...
					if (str.getClass().equals(JSONObject.class)) {
						Object child = toClass((JSONObject) str, type);
						if (child != null)
							f.set(ob, child);
					}
				}
	 ...
	 */
	public Object toClass(String obj, Class<?> c) {
		JSONObject object = parseObject(obj);
		...
		if (null == fs)
			return ob;
		return toClass(object, ob, fs, c.getName());
	}
	 ...
	 * @return Class对象 错误 return null
	 */
	public Object toClass(JSONObject object, Class<?> c) {
	...
	public <T> String toString(List<T> datas) {
		if (datas == null || datas.size() < 0)
			return "[]";
		String result = "[";
		for (int i = 0; i < datas.size(); i++) {
			result += toJSONString(datas.get(i)) + ",";
		}
...
//				Log.i("result", type.getSimpleName() + " " + name + " 匹配失败!");
	...
	/** 拼接json中间字段,无头尾{}。*/
	public String add2Json(String key, Object value){
		...
		if (type.equals(String.class) || type.equals(char.class)) {
			...
			String str = toString((List<Object>) value);
	...
	/**是否是基本类型 */
	public boolean isBaseType(Class<?> c){
	...
	/** 获取制定元素位置 */
	public int getIndex(JSONArray array,Object object){
	 ...
	 * @return 获取某个字段
	 */
	public String getData(String result,String key) {
		JSONObject ob = parseObject(result);
		if (null == ob)
			return "";
	 ...
	 * @return 获取Array数据
	 */
	public JSONArray getDataArray(String result,String key) {
		JSONObject ob = parseObject(result);
		if (null == ob)
			return null;
	 ...
	 * @return 获取Object数据
	 */
	public JSONObject getDataObject(String result,String key) {
		JSONObject ob = parseObject(result);
		if (null == ob)
			return null;
	 ...
	 * @return	状态是否成功
	 */
	public boolean getState(String result){
		String stat = getData(result,"code");
		if("0".equals(stat))
			return true;
		return false;
	}
	/** 获取data数据 */
	public String getData(String result){return getData(result,"data");	}
