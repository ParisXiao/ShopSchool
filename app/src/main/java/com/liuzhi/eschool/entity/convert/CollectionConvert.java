...
public class CollectionConvert implements Converter<CollectionEntity> {

    @Override
    public CollectionEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,CollectionEntity.class);
    }
}
