...
public class ClassTypeConvert implements Converter<ClassTypeEntity> {

    @Override
    public ClassTypeEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ClassTypeEntity.class);
    }
}
