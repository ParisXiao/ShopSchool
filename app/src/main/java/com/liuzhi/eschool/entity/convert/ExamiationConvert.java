...
public class ExamiationConvert implements Converter<ExamiationEntity> {

    @Override
    public ExamiationEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ExamiationEntity.class);
    }
}
