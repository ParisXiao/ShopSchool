...
public class FinishWorkConvert implements Converter<FinishWorkEntity> {

    @Override
    public FinishWorkEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,FinishWorkEntity.class);
    }
}
