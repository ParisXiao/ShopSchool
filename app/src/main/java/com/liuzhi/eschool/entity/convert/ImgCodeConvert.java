...
public class ImgCodeConvert implements Converter<ImgCodeEntity> {

    @Override
    public ImgCodeEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ImgCodeEntity.class);
    }
}
