...
public class ClassConvert implements Converter<ClassEntity> {

    @Override
    public ClassEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ClassEntity.class);
    }
}
