...
public class ClassSearchConvert implements Converter<ClassSearchEntity> {

    @Override
    public ClassSearchEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ClassSearchEntity.class);
    }
}
