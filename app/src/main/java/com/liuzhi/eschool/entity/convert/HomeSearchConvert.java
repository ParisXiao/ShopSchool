...
public class HomeSearchConvert implements Converter<HomeSearchEntity> {

    @Override
    public HomeSearchEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,HomeSearchEntity.class);
    }
}
