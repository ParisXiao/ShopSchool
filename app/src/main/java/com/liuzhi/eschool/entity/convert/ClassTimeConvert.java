...
public class ClassTimeConvert implements Converter<ClassTimeEntity> {

    @Override
    public ClassTimeEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ClassTimeEntity.class);
    }
}
