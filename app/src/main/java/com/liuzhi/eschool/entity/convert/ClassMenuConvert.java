...
public class ClassMenuConvert implements Converter<ClassMenuEntity> {

    @Override
    public ClassMenuEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,ClassMenuEntity.class);
    }
}
