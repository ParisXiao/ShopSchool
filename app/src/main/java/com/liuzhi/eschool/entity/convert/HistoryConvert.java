...
public class HistoryConvert implements Converter<HistoryEntity> {

    @Override
    public HistoryEntity convertResponse(Response response) throws Throwable {
        ...
        if (TextUtils.isEmpty(data) || !JsonUtils.getInstance().isJsonObj(data))
            return null;
        return AppConsds.mGson.fromJson(data,HistoryEntity.class);
    }
}
